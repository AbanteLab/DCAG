...
#     dataset_path="/pool01/projects/abante_lab/cag_propagation/handsaker_deepdive10X_thompson2020.h5", #handsaker labeled, thompson unlabeled
#     # dataset_path="/pool01/projects/abante_lab/cag_propagation/multitech_supervised_adata_v2_6000.h5",
#     batch_size=1000,        # FIXME: issue when it's too large?????
#     cuda=True,
#     multitech = True,
...
#%%
    ...
    # Train containing labeled and unlabeled cells, test containing labeled cells for performance evaluation
...
#%%
    ...
    # NOTE: Skipping normalization of count matrix by gene length

    # Train base scVI
    scvi.model.SCVI.setup_anndata(train_adata, layer="raw", batch_key="DONOR")
    ...
    label_vals = train_adata.obs["CELLTYPE"].dropna().values  # labelled cells only
    ...
    if args.plot:
        plot_scanvi_pca_latent(scanvi_model, train_adata, latent="Z", output_path=f"{output_path}{model_sufix}_pca_z_trainset.pdf", show_plot=True, multitech=args.multitech)
        plot_scanvi_pca_latent(scanvi_model, test_adata, latent="Z", output_path=f"{output_path}{model_sufix}_pca_z_testset.pdf", show_plot=True, multitech=args.multitech)

        plot_losses_scanvi(scanvi_model, f"{output_path}{model_sufix}_losses.pdf", show_plot=True)

    evaluate_model(scanvi_model, train_adata, test_adata, args, dataset_name, output_path=output_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--num-epochs", default=5000, type=int, help="number of training epochs"
    )
    parser.add_argument(
        "-d",
        "--dataset_path",
...
# train_adata.obsm[SCVI_LATENT_KEY] = scvi_model.get_latent_representation()
# sc.pp.neighbors(train_adata, use_rep=SCVI_LATENT_KEY)
...
# )
# #%%
# sc.pp.neighbors(train_adata, use_rep=SCANVI_LATENT_KEY)
# sc.tl.umap(train_adata, min_dist=0.3)

# # in order to make colors matchup
# train_adata.obs.C_scANVI = pd.Categorical(
#     train_adata.obs.C_scANVI.values, categories=train_adata.obs.CAGPHASE.cat.categories
# )
# sc.pl.umap(
#     train_adata,
#     color=["CAGPHASE", SCANVI_PREDICTION_KEY],
#     frameon=False,
#     ncols=1
# )

# #%%
# sc.pp.neighbors(test_adata, use_rep=SCANVI_LATENT_KEY)
# sc.tl.umap(test_adata, min_dist=0.3)

# # in order to make colors matchup
# test_adata.obs.C_scANVI = pd.Categorical(
#     test_adata.obs.C_scANVI.values, categories=test_adata.obs.CAGPHASE.cat.categories
# )
# sc.pl.umap(
#     test_adata,
#     color=["CAGPHASE", SCANVI_PREDICTION_KEY],
